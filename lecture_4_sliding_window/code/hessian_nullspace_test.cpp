...
int main() {
    ...
    for (int j = 0; j < featureNums; ++j) {
        ...
        for (int i = 0; i < poseNums; ++i) {
            ...
            ///补充完整息矩阵块的计算
            H.block(i * 6, poseNums * 6 + j * 3, 6, 3)
                    += jacobian_Ti.transpose() * jacobian_Pj;
            H.block(poseNums * 6 + j * 3, i * 6, 3, 6)
                    += (jacobian_Ti.transpose() * jacobian_Pj).transpose();
    ...
    std::cout << "Full BA: " << svd.singularValues().transpose() << std::endl << std::endl;

    Eigen::MatrixXd H_block = H.block(6, 6, diem - 6, diem - 6);
    Eigen::JacobiSVD<Eigen::MatrixXd> svd_block(H_block, Eigen::ComputeThinU | Eigen::ComputeThinV);
    std::cout << "Fix a pose: " << svd_block.singularValues().transpose() << std::endl;
